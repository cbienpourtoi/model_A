...
pro do_alpha,pne_cat,file_with_rad,sersic_par,distanceMpc,output
;,extinction=extinction, ellipt=ellipt,alpha_1=alpha_1,alpha_25=alpha_25
...
for i = 0, n_elements(rbin)-1 do begin
   ...
   indici = where(rpne gt rlow[i] and rpne le rup[i])
...
boundaries_max=[min(rlow),max(rup)]
boundaries_min=[min(rlow),max(rup)]
...
total_luminosity_1=integrate_sersic(sersic_par[0],m,sersic_par[2],sersic_par[3],md,r_min1)
;/(10.^(-0.4*solar_mag))
total_luminosity_2=integrate_sersic(sersic_par[0],m,sersic_par[2],sersic_par[3],md,r_max1)
;/(10.^(-0.4*solar_mag))
total_luminosity_max = (total_luminosity_2 - total_luminosity_1)
;/(10.^(-0.4*extinction))

total_luminosity_1=integrate_sersic(sersic_par[0],m,sersic_par[2],sersic_par[3],md,r_min2)
;/(10.^(-0.4*solar_mag))
total_luminosity_2=integrate_sersic(sersic_par[0],m,sersic_par[2],sersic_par[3],md,r_max2)
;/(10.^(-0.4*solar_mag))
total_luminosity_min = (total_luminosity_2 - total_luminosity_1)
;/(10.^(-0.4*extinction))
...
pro alpha_ari,norm,mstar
pne_cat='Kall.dat'
file_with_rad='bin_model9.dat'
;completeness='1023.completeness.dat'
output='A.alpha'
;m80file='m80.dat';
sersic_par=[160., 20.31, 4, 54, 18]  ;add color to move from R-band to B-band B=R+1.6
;a=0.22  ;extinction in B band according to NED (a=0.132 R) 0.22 B
distanceMpc=10.
;m_b_sun=5.48
;m_v_sun=4.83

do_alpha,pne_cat,file_with_rad,sersic_par,distanceMpc,output
;,extinction=a, ellipt=ellipt,alpha_1=alpha_1,alpha_25=alpha_25
